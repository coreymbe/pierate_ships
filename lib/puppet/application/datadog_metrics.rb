...
class Puppet::Application::Datadog_agent < Puppet::Application
  ...
  def extract_data(final_data, data, path)
    if path.count == 1
      if data[path[0]].nil?
        Puppet.debug "ERROR with last FILTER KEY; Check your filter parameter"
      end
      ...
      begin
        ...
        Puppet.debug "Potential ERROR with middle FILTER KEY: #{e.backtrace}"
      end
    end
  end
  ...
  def parse_metrics(metrics, server, service, filters)
    ...
    if service == 'puppetserver'
      ...
      jruby_metrics['borrow-timers'].each do |endpoint, metrics|
        metrics.each do |metric, value|
          all_metrics["borrow-timers.#{endpoint}.#{metric}"] = value
        end
      end
    end
